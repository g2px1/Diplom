...
test::framework::Generator::Generator(uint8_t push_percantage, uint8_t pop_percantage, uint8_t read_percantage,
    ...
    if (total_distribution != 100) {
        std::cerr << "Wrong distribution: " << (int) total_distribution << "%, should be 100%";
    }
    std::random_device rd;
    this->gen_engine = std::mt19937_64(rd());
}

void test::framework::Generator::generator(bool writeLogFile, const std::string &file_name) {
    ...
    double write_operation_q = static_cast<double>(operations_q) - push_operation_q - pop_operation_q - read_operation_q;

    std::vector<int16_t> operations;
    operations.reserve(this->operations_q);

    std::discrete_distribution<> d({push_operation_q, pop_operation_q, read_operation_q, write_operation_q});
